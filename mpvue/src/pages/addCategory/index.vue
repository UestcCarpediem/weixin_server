...
<template>
  <div class="longinPage">
    <div
      url=""
      class="weui-cell weui-cell_access plan_list_phtot"
      hover-class="weui-cell_active"
      @click.stop="chooseImage(1)"
    >
      <div>
          ...
          <div class="weui-uploader__files" id="uploaderFiles">
            <block v-for="(item, index) in files" :key="index">
              <div
                ...
                @click.stop="predivImage($event, 1)"
                :id="item"
              ></div>
            </block>
            <image
              class="weui-uploader__img plan_photo"
              :src="photoUrl"
              mode="aspectFill"
            />
          </div>
...
export default {
  ...
  methods: {
    ...
    displayContent() {
      this.api
        .getUserDetails({ openid: wx.getStorageSync("openid") })
        .then((res) => {
          if (res.code == 1) {
    ...
    complete() {
      ...
      if (!this.formList.photoUrl) {
        ...
        let stamp = this.formList.date + " " + this.formList.time;
        let endstamp = this.formList.enddate + " " + this.formList.endtime;
        let duration = new Date(endstamp).getTime() - new Date(stamp).getTime();
        console.log(duration, 3 * 24 * 60 * 60 * 1000);
