  ...
  <div class="my-wrap">
      ...
      <div class="my-info">
        <div class="my-info-item my-info-item-1">
          ...
          <div class="item-bottom">预约次数</div>
        </div>
        <div class="my-info-item my-info-item-2">
          <div class="item-top">{{success}}</div>
          <div class="item-bottom">成功次数</div>
        </div>
      </div>
      
      <div class="my-list">
        <div class="list-item" @click="navigateTo('/pages/categoryList/main')">
          <p class="list-item-left"><i class="iconfont icon--biaodanfenlei"></i>个人资料</p>
...
export default {
  ...
  methods: {
    getCount(){
      console.log(222)
      this.api.getRecordCount({"openid":wx.getStorageSync('openid')}).then((res)=>{
        this.total=res.data.dataTotal
        this.success=res.data.dataPending
        console.log(res)
      })
    },
    navigateTo(url){
